...
class NFT {
    ...
    async initialize(network) {
        const { collectionId, collectionIndex, collectionName, nftCodeBalance, nftP2pkhBalance, nftName, nftSymbol, nft_attributes, nftDescription, nftTransferTimeCount, nftIcon } = await API.fetchNFTInfo(this.contract_id, network);
        let file = "";
    ...
    static async createCollection(address, privateKey, data, utxos, network) {
    ...
    static async createNFT(collection_id, address, privateKey, data, utxos, network) {
    ...
    async transferNFT(address_from, address_to, privateKey, utxos, network) {
